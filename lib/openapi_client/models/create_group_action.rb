  ...
  class CreateGroupAction
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::CreateGroupAction` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::CreateGroupAction`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'chat_ids')
        if (value = attributes[:'chat_ids']).is_a?(Array)
          self.chat_ids = value
        end
      end
      ...
      if attributes.key?(:'message_text')
        self.message_text = attributes[:'message_text']
      end
    end
    ...
    def hash
      [group_name, chat_ids, phones, message_text].hash
    end
